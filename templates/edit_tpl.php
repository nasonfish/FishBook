...
<form method="post" action="/edit.php" class="hr-gray">
    ...
    <hr/>
    <label for="tags">Enter things you would like to tag this user by. This will allow you to filter users to build lists.</label> <a class="js-click" onclick='$(this).after("<input type=\"text\" name=\"tags[]\">");'>(Add new tag (tag as many things as you can!))</a>
    <?php foreach($user->getTags() as $tag): ?>
    <input type="text" name="tags[]" id="tags" value="<?=$tag?>"/>
    <?php endforeach; ?>
    <hr/>
    <label for="notes">Add extra notes about this person! (use {person name} to link to a person, such as a parent or related person)</label> <a class="js-click" onclick='$(this).after("<input type=\"text\" name=\"notes[]\">");'>(Add new note)</a>
    <?php foreach($user->getNotes() as $note): ?>
    <input type="text" name="notes[]" id="notes" value="<?=$note?>"/>
    <?php endforeach; ?>
    <hr/>
    <label for="contact">Add more contact information about this person</label> <a class="js-click" onclick='$(this).after("<input type=\"text\" name=\"contact[]\">");'>(Add new contact note)</a>
